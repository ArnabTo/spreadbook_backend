...
def _get_product_for_invoice_item(obj: InvoiceItem | None):
    ...
    try:
        return (
            ...
            .select_related("unit")
            .only("priceSale", "supplier_price", "unit__name")
            .first()
        )
    except Exception:
        return None
...
class POSOrderItemSerializer(serializers.ModelSerializer):
    ...
    selling_unit_conversion_factor = serializers.SerializerMethodField(read_only=True)
...
class POSOrderSerializer(serializers.ModelSerializer):
    ...
    # New receipt header fields from company
    company_title_line1 = serializers.SerializerMethodField()
    company_title_line2 = serializers.SerializerMethodField()
    ...
    def get_company_title_line1(self, obj):
        """Get first subtitle line from company for receipt header."""
        company = getattr(obj, "companyId", None) or getattr(
            getattr(obj, "branch", None), "company", None
        )
        return getattr(company, "company_title_line1", None) if company else None

    def get_company_title_line2(self, obj):
        """Get second subtitle line from company for receipt header."""
        company = getattr(obj, "companyId", None) or getattr(
            getattr(obj, "branch", None), "company", None
        )
        return getattr(company, "company_title_line2", None) if company else None

    def get_company_website(self, obj):
        """Get company website from company for receipt header."""
        company = getattr(obj, "companyId", None) or getattr(
            getattr(obj, "branch", None), "company", None
        )
        if not company:
            return None
        return getattr(company, "company_website", None) or getattr(
            company, "url", None
        )

    def get_branch_address(self, obj):
        """Get branch address for receipt header."""
        branch = getattr(obj, "branch", None)
        if branch:
            return getattr(branch, "fullAddress", None) or getattr(
                branch, "location", None
            )
        return None

    def get_branch_phone(self, obj):
        """Get branch phone for receipt header."""
        branch = getattr(obj, "branch", None)
        if branch:
            return getattr(branch, "phone", None) or getattr(
                branch, "phoneNumber", None
            )
        return None
                ...
                if product:
                    ...
                    if selling_unit_qty_dec is not None and product_unit is not None:
                        ...
                        if getattr(product_unit, "price", None) is not None:
                            try:
                                pu_price = Decimal(str(product_unit.price or 0))
                                if pu_price > 0:
                    ...
                    if variant_obj is not None:
                        ...
                        current_stock = int(
                            _PV.objects.filter(product=product).aggregate(
                                total=_Sum("size_qty")
                            )["total"]
                            or 0
                        )
