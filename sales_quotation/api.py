...
def _generate_bill_number(company, financial_year=None) -> str:
    """Generate the next quotation number atomically using the Prefix system.

    Falls back to SQ-<n> within the company if no Prefix record exists.
    """
    ...
    qs = Prefix.objects.filter(
        company=company, type="sales_quotation", applicable=True
    )
    ...
    with transaction.atomic():
        ...
        if prefix_obj is None:
            # Fall back: use the highest existing bill_number in this company.
            last_qs = (
                SalesQuotation.objects.filter(companyId=company)
                .order_by("-created_at")
        ...
        # Pad based on start_index width if reasonable; default 3.
        try:
...
class SalesQuotationPageNumberPagination(PageNumberPagination):
    page_size = 10
    ...
    def perform_create(self, serializer):
        ...
        if is_unrestricted_user(user):
            ...
            if ids:
                ...
                company = Company.objects.filter(id__in=list(ids)).first()
        serializer.save(companyId=company, user=user)


class BankAccountViewSet(viewsets.ReadOnlyModelViewSet):
    serializer_class = BankAccountLiteSerializer
    permission_classes = [IsAuthenticated]
...
class SalesQuotationViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = SalesQuotationPageNumberPagination

    def get_serializer_class(self):
        if self.action in ("create", "update", "partial_update"):
            return SalesQuotationWriteSerializer
    ...
    def get_queryset(self):
        qs = (
            ...
            .prefetch_related(
                Prefetch(
                    "items",
                    queryset=SalesQuotationItem.objects.select_related(
                        "product", "unit"
                    ),
                )
            )
        ...
        rfq = params.get("rfq_ref")
        if rfq:
            qs = qs.filter(rfq_ref__icontains=rfq)
    ...
    def _recalc_totals(self, quotation: SalesQuotation, items_data, discount_total=None):
        ...
        if discount_total is None:
            try:
                discount_total = Decimal(str(self.request.data.get("discount_total") or 0))
            except Exception:
                discount_total = Decimal("0")
        grand_total = total + tax_total - discount_total
    ...
    def _get_products_payload(self, request):
        """Return products list as a Python list (parsed from JSON string if needed)."""
        # request.data may be either a DRF Request (dict-like) or a QueryDict
        products = None
        try:
    ...
    def _build_request_data_for_serializer(self, request, products_list):
        """Build a mutable dict for serializer input with products replaced as a list.

        DRF QueryDict does not cleanly support list values for non-multi fields. We
        build a fresh dict from request.data and replace 'products' so the
        ListField(child=DictField) receives a real Python list.
        """
        data = {}
        try:
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            quotation = SalesQuotation(companyId=company, branch=branch, user=user)
            ...
            if attachment is not None:
                quotation.attachment = attachment
            # bill_number auto-generation
            quotation.bill_number = _generate_bill_number(company, fy)
            quotation.financial_year = fy
            # Recalc totals from items
            item_objs = self._recalc_totals(quotation, items_data)
            quotation.save()

            # Bulk create items
            for item in item_objs:
                item.quotation = quotation
            SalesQuotationItem.objects.bulk_create(item_objs)

        out = SalesQuotationDetailSerializer(quotation).data
    ...
    def update(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if items_data is not None:
                discount_total = validated.get("discount_total")
                self._recalc_totals(instance, items_data, discount_total=discount_total)
            instance.save()
            ...
            if items_data is not None:
                ...
                for it in new_items:
                    it.quotation = instance
                SalesQuotationItem.objects.bulk_create(new_items)

        out = SalesQuotationDetailSerializer(instance).data
        return Response(out)
    ...
    def perform_destroy(self, instance):
        ...
        if not is_unrestricted_user(user):
            ...
            if not ids or str(instance.companyId_id) not in [str(i) for i in ids]:
                raise PermissionDenied("You cannot delete this quotation")
        instance.delete()
        ...
        return Response(
            {
                ...
                "bank_accounts": BankAccountLiteSerializer(
