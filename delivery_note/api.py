...
def _generate_bill_number(company, financial_year=None) -> str:
    """Generate the next delivery note number atomically using the Prefix system.

    Falls back to DN-<n> within the company if no Prefix record exists.
    """
    ...
    qs = Prefix.objects.filter(
        company=company, type="delivery_note", applicable=True
    )
    ...
    with transaction.atomic():
        ...
        if prefix_obj is None:
            last_qs = (
                DeliveryNote.objects.filter(companyId=company)
                .order_by("-created_at")
...
class DeliveryNotePageNumberPagination(PageNumberPagination):
    page_size = 10
...
class DeliveryNoteViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = DeliveryNotePageNumberPagination
    ...
    def get_queryset(self):
        qs = (
            ...
            .prefetch_related(
                Prefetch(
                    "items",
                    queryset=DeliveryNoteItem.objects.select_related(
                        "product", "unit"
                    ),
                )
            )
    ...
    def _recalc_totals(self, note: DeliveryNote, items_data):
        ...
        note.total = total
        note.tax_total = tax_total
        note.product_discount_total = product_discount_total
        note.cash_discount_total = cash_discount_total
        note.grand_total = grand_total
        return items
    ...
    def create(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            note = DeliveryNote(companyId=company, branch=branch, user=user)
            ...
            note.bill_number = _generate_bill_number(company, fy)
            note.financial_year = fy
            item_objs = self._recalc_totals(note, items_data)
            note.save()
        ...
        out = DeliveryNoteDetailSerializer(note).data
    ...
    def update(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if items_data is not None:
                ...
                for it in new_items:
                    it.delivery_note = instance
                DeliveryNoteItem.objects.bulk_create(new_items)

        out = DeliveryNoteDetailSerializer(instance).data
        return Response(out)
    ...
    def perform_destroy(self, instance):
        ...
        if not is_unrestricted_user(user):
            ...
            if not ids or str(instance.companyId_id) not in [str(i) for i in ids]:
                raise PermissionDenied("You cannot delete this delivery note")
        instance.delete()
